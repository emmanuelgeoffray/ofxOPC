...
#include "ofxOPC.h"
//--------------------------------------------------------------
void ofxOPC::setup(string address, int port, int fadecandyCount)
{
    // Copy the Address and port to the variables
    _port = port;
    _address = address;
    _fadecandyCount = fadecandyCount;
    ...
    uint16_t data_length = fadecandyCount * 8 * 64 * sizeof(OPCPacket_SPCData_t);
